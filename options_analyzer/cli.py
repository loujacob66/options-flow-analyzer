...
def analyze(
    ...
    expiration: Optional[str] = typer.Option(None, "--expiration", "-e", help="Expiration date in YYYY-MM-DD format"),
    min_volume: int = typer.Option(10, "--min-volume", "-v", help="Minimum volume for filtering"),
    option_type: Optional[str] = typer.Option(None, "--option-type", "-t", help="Option type: calls or puts"),
    show_unusual: bool = typer.Option(True, "--unusual/--no-unusual", help="Show unusual activity analysis"),
    show_max_pain: bool = typer.Option(True, "--max-pain/--no-max-pain", help="Show max pain analysis"),
    detect_sweeps: bool = typer.Option(True, "--sweeps/--no-sweeps", help="Detect and analyze sweep trades"),
    multiple_expirations: bool = typer.Option(False, "--multi-exp", "-m", help="Analyze multiple expirations")
):
    ...
    try:
        ...
        if 'error' in ticker_info:
            display.show_error("All data sources failed. Try using demo mode: python -m options_analyzer demo")
            return
        ...
        if multiple_expirations:
            display.show_warning("Multiple expirations not fully supported with Polygon API yet")
        ...
        display.show_success(f"Found {len(filtered_data)} option contracts matching criteria")
        ...
        if detect_sweeps:
            ...
            if not clean_data.empty:
                ...
                strike_analysis = analyzer.analyze_strike_distribution(clean_data, current_price)
                ...
                strike_analysis = analyzer.analyze_strike_distribution(filtered_data, current_price)
        else:
...
def demo(
    ...
    min_volume: int = typer.Option(50, "--min-volume", "-v", help="Minimum volume for filtering")
):
