...
class OptionsDisplay:
    ...
    def show_ticker_info(self, ticker_info: Dict[str, Any]):
        ...
        if 'error' in ticker_info:
            self.console.print(f"[red]Error fetching data for {ticker_info['symbol']}: {ticker_info['error']}[/red]")
            return
...
"""
    ...
    def show_flow_summary(self, flow_summary: Dict[str, Any]):
        ...
        table = Table(title="Options Flow Summary", show_header=True, header_style="bold magenta")
...
"""
    ...
    def show_strike_analysis(self, strike_df: pd.DataFrame, max_rows: int = 15):
        ...
        table = Table(title="Top Strikes by Volume", show_header=True, header_style="bold magenta")
...
f"{row['moneyness']}"
    ...
    def show_unusual_activity(self, unusual_df: pd.DataFrame, max_rows: int = 10):
        ...
        table = Table(title="Unusual Activity (High Volume/OI Ratio)", show_header=True, header_style="bold red")
        table.add_column("Strike", justify="right")
        table.add_column("Type", justify="center")
...
"""
        self.console.print(Panel(max_pain_text, title="Max Pain Analysis", border_style="purple"))
        
        # Top strikes by OI
        table = Table(title="Open Interest by Strike", show_header=True, header_style="bold purple")
        table.add_column("Strike", justify="right")
        table.add_column("Open Interest", justify="right")
        table.add_column("Volume", justify="right")
    ...
    def show_expiration_analysis(self, exp_df: pd.DataFrame):
        ...
        table = Table(title="Flow by Expiration", show_header=True, header_style="bold cyan")
        table.add_column("Expiration")
        table.add_column("Volume", justify="right")
        table.add_column("Dollar Flow", justify="right")
...
"""
        ...
        self.console.print(Panel(sweep_summary, title="🔍 Sweep Analysis", border_style="yellow"))
        ...
        if all_trades and without_sweeps:
            table = Table(title="Flow Analysis: With vs Without Sweeps", show_header=True, header_style="bold cyan")
            table.add_column("Metric", style="cyan")
            ...
            all_sentiment = "🟢 Bullish" if all_trades.get('bullish_sentiment', False) else "🔴 Bearish"
            clean_sentiment = "🟢 Bullish" if without_sweeps.get('bullish_sentiment', False) else "🔴 Bearish"
            sentiment_change = "⚠️ Changed" if impact.get('sentiment_change', False) else "✓ Same"
...
"""
            self.console.print(Panel(sweep_info, title="Sweep-Only Analysis", border_style="red"))
    ...
    def show_trade_classification(self, df: pd.DataFrame, max_rows: int = 10):
        ...
        count_table = Table(title="Trade Classification", show_header=True, header_style="bold purple")
        ...
        sweeps = df[df['trade_type'] == 'sweep'].sort_values('sweep_confidence', ascending=False)
        if not sweeps.empty:
            sweep_table = Table(title="Top Detected Sweeps", show_header=True, header_style="bold red")
