...
class OptionsAnalyzer:
    ...
    def analyze_strike_distribution(self, df: pd.DataFrame, current_price: float) -> pd.DataFrame:
        """
        ...
        strike_analysis = df.groupby(['strike', 'option_type']).agg({
            'volume': 'sum',
            'openInterest': 'sum',
            'dollar_flow': 'sum',
            'lastPrice': 'mean'
        ...
        strike_analysis['distance_from_price'] = strike_analysis['strike'] - current_price
        strike_analysis['distance_pct'] = (strike_analysis['distance_from_price'] / current_price) * 100
    ...
    def find_max_pain(self, df: pd.DataFrame) -> Tuple[float, pd.DataFrame]:
        ...
        max_pain_strike = oi_by_strike.loc[oi_by_strike['openInterest'].idxmax(), 'strike']
        
        return max_pain_strike, oi_by_strike.sort_values('openInterest', ascending=False)
    ...
    def analyze_expiration_flow(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        exp_analysis = df.groupby(['expiration', 'option_type']).agg({
            'volume': 'sum',
            'openInterest': 'sum',
            'dollar_flow': 'sum'
        }).reset_index()
        ...
        exp_totals = df.groupby('expiration').agg({
            'volume': 'sum',
            'dollar_flow': 'sum'
        }).reset_index()
    ...
    def identify_unusual_activity(self, df: pd.DataFrame, volume_threshold: float = 2.0) -> pd.DataFrame:
        """
        ...
        df_copy = df.copy()
        df_copy['volume_oi_ratio'] = df_copy['volume'] / (df_copy['openInterest'] + 1)  # +1 to avoid division by zero
        
        # Filter for unusual activity
        unusual = df_copy[df_copy['volume_oi_ratio'] >= volume_threshold]
        
        # Sort by volume and dollar flow
        unusual = unusual.sort_values(['volume', 'dollar_flow'], ascending=[False, False])
        
        return unusual[['strike', 'option_type', 'expiration', 'volume', 'openInterest', 
                      'volume_oi_ratio', 'dollar_flow', 'lastPrice']]
    
    def calculate_gamma_exposure(self, df: pd.DataFrame, current_price: float) -> Dict[str, float]:
        """
        ...
        gamma_exposure = {}
        price_levels = [current_price * (1 + i * 0.01) for i in range(-10, 11)]  # ±10% in 1% increments
        ...
        for price in price_levels:
            ...
            for _, row in df.iterrows():
                ...
                if 0.9 <= moneyness <= 1.1:  # Near the money options have higher gamma
                    gamma_contribution = row['openInterest'] * (1 - abs(moneyness - 1) * 5)
                    if row['option_type'] == 'call':
    ...
    def detect_sweeps(self, df: pd.DataFrame, sweep_threshold: float = 0.5) -> pd.DataFrame:
        """
        ...
        def calculate_sweep_confidence(row):
            ...
            dollar_score = min(row['dollar_flow'] / df_copy['dollar_flow'].quantile(0.95), 2.0) / 2.0
    ...
    def analyze_without_sweeps(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        clean_summary = self.calculate_flow_summary(non_sweep_df) if not non_sweep_df.empty else {}
        
        # Sweep-only analysis
        sweep_df = df[df['trade_type'] == 'sweep'].copy()
        sweep_summary = self.calculate_flow_summary(sweep_df) if not sweep_df.empty else {}
        
        # Calculate the impact of removing sweeps
        impact_analysis = {}
        if all_trades_summary and clean_summary:
            impact_analysis = {
                'volume_change': clean_summary.get('net_volume', 0) - all_trades_summary.get('net_volume', 0),
                'dollar_flow_change': clean_summary.get('net_dollar_flow', 0) - all_trades_summary.get('net_dollar_flow', 0),
                'sentiment_change': clean_summary.get('bullish_sentiment', False) != all_trades_summary.get('bullish_sentiment', False)
            }
